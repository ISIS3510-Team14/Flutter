...
Future<Map<String, dynamic>> getAnswer(String imagePath) async {
  ...
  List<TrashTypeIcon> trashTypes = [
    ...
    TrashTypeIcon('none', Icons.announcement_outlined),
  ];
  ...
  for (TrashTypeIcon trash in trashTypes) {
    if (openaiAnswer!.contains(trash.type.toLowerCase())) {
      foundTrashType = trash.type;
      if (foundTrashType!="none"){
        openaiAnswer = await ChatService().request("Answer for the image: What is the most appropriate bin to dispose of a $foundTrashType in?. Indicate if none of the present bins are appropriate. Answer only with a maximum of 20 words.", imageBase64);
